...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
		...
		config.dnsmasq.domain = settings["dnsmasq_domain"]
		config.dnsmasq.ip = settings["dnsmasq_ip"]
		...
		if OS.mac?
			...
			config.dnsmasq.reload_command = 'sudo brew services dnsmasq reload'
		end
		...
		settings['servers'].each do |servers|
			config.vm.define servers["name"] do |srv|
				...
				srv.vm.synced_folder "./","/home/vagrant/example", {:mount_options => ['dmode=777','fmode=777']}
